...
def batch_ogg():
    ...
    print(f"🍃 Mengonversi {len(files)} file ke OGG...")
    for filename in tqdm(files, desc="Progress OGG", unit="file"):
        try:
            path_in = os.path.join(input_folder, filename)
            path_out = os.path.join(output_folder, f"{os.path.splitext(filename)[0]}.ogg")
            
            audio = AudioSegment.from_mp3(path_in)
            audio.export(path_out, format="ogg")
